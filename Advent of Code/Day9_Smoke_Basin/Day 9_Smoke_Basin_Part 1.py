...
for i in range(len(test_depth_data)):
    for j in range(len(test_depth_data[0])):
        if j != 0:
            if test_depth_data[i][j] < test_depth_data[i][j-1]: ##
                checker[0] = True
        else:
            checker[0] = True
        if j != len(test_depth_data[0])-1:
            if test_depth_data[i][j] < test_depth_data[i][j+1]: ##
                checker[1] = True
        else:
            checker[1] = True
        if i != 0:
            if test_depth_data[i][j] < test_depth_data[i-1][j]: ##
                checker[2] = True
        else:
            checker[2] = True
        if i != len(test_depth_data)-1:
            if test_depth_data[i][j] < test_depth_data[i+1][j]: ##
                checker[3] = True
        else:
            checker[3] = True
        ...
        if checker == [True, True, True, True]:
            trophs.append(test_depth_data[i][j])
            risk += test_depth_data[i][j]+1
        checker = [False, False, False, False]
